...
export default function TestBillList() {
  const [bills, setBills] = useLocalStorage('elab_bills', initialDummyBills);
  const [showExport, setShowExport] = useState(false);

  const handleDelete = (billId) => {
     if(confirm('Cancel this Invoice?')) {
       setBills(bills.filter(b => b.bill !== billId));
     }
  }
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="p-2 border-b-2 border-slate-400 flex gap-2 bg-slate-200 justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <div className="relative w-64 border border-slate-400 bg-white">
              ...
              <input type="text" className="w-full text-xs py-1 pl-6 pr-2 rounded-none focus:ring-0 focus:outline-none border-none" />
            </div>
          </div>
          <div className="text-xs font-bold text-slate-700">Total Invoices: {bills.length}</div>
        </div>
          ...
          <table className="min-w-full divide-y divide-slate-300">
            <thead className="bg-slate-300">
              <tr>
                ...
                <th scope="col" className="px-2 py-1 text-center text-xs font-bold text-slate-800 uppercase w-24">Action</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-slate-200">
              {bills.map((bill, idx) => (
                <tr key={idx} className="hover:bg-blue-50">
                  <td className="px-2 py-1 border-r border-slate-300 text-center text-xs text-slate-600">{idx + 1}</td>
                  ...
                  <td className="px-2 py-1 text-center text-xs flex gap-2 justify-center">
                    ...
                    <button onClick={() => handleDelete(bill.bill)} className="text-red-700 font-bold hover:underline">Del</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <ExportModal isOpen={showExport} onClose={() => setShowExport(false)} entityName="Invoice Billing Ledgers" />
    </div>
  );
}
